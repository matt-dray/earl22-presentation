# Prepare egg statistics
# Creates objects to pass to {a11ytables} in EARL presentation
# Matt Dray, Aug 2022
...
tables_list <- c(qtrly_mutated, yrly_mutated) |>
  map(
    ~mutate(
      .x, 
...
cover_df <- tribble(
  ...
  "Revisions policy", paste0("Figures in this dataset are provisional and subject to revision. We will provide information about any revisions we make to previously published information in this dataset, and the associated statistics notice. Revisions could occur for various reasons including:",
                             ...
                             "\n  - we may also revise the methodology used to raise the survey data to give UK totals"),
...
notes_df <- tribble(
  ...
  "[note 1]", "All eggs from free range and organic flocks have been re-labelled as barn eggs since 21st March 22 due to avian influenza restrictions.",
  "[note 2]", "Prices up to and including 2011 are a period end price and exclude bonus payments.",
  "[note 3]", "From Q1 2012 prices shown are based on an average across the period and include bonus payments.",
  "[note 4]", "From Q1 2006 the average UK All Types Price includes Organic eggs."
)
